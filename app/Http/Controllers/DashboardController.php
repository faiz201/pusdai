...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $distribusi = [
            ...
            'Pemantauan Pemenuhan Kewajiban LHK' => LHK::where(fn($q) => $applyFilter($q))->count(),
            'Pelaksanaan Monev ZI' => Pemantauan::where(fn($q) => $applyFilter($q))->count(),
            'Analisis Data Informasi Pegawai' => Pegawai::where(fn($q) => $applyFilter($q))->count(),
            'Hasil Survey Kinerja Organisasi' => HasilSurvei::where(fn($q) => $applyFilter($q))->count(),
            'Penanganan Pengaduan Masyarakat' => Pengaduan::where(fn($q) => $applyFilter($q))->count(),
        ];
